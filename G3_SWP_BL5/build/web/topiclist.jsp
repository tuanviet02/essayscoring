        ...
        <main class="main-content position-relative max-height-vh-100 h-100 border-radius-lg">
                    ...
                    <nav aria-label="breadcrumb">
                        <ol class="breadcrumb bg-transparent mb-0 pb-0 pt-1 px-0 me-sm-6 me-5">
                            ...
                            <li class="breadcrumb-item text-sm text-dark active" aria-current="page">
                                <a class="opacity-5 font-weight-bolder" href="topiclist">Topic List</a>
                            </li>
                        </ol>
                    </nav>
            ...
            <div class="container-fluid py-4">
                <div class="row">
                    <div class="col-xl-6">
                        <div class="row">
                            <form action="topicsearch" class="col-md-5 col-5">                 
                                <label>Search</label>
                            ...
                            <form action="topicfilter" id="filter" class="row">  
                                    ...
                                    <div class="card-header pb-0 p-3">
                                        <div class="row">
                                            ...
                                            <div class="col-6 text-end">
                                                <a class="btn bg-gradient-dark mb-0" href="addtopic">
                                                    Add New Topic
                                                </a>
                                            </div>
                                        </div>
                                        ...
                                        <div class="table-responsive p-0">
                                                            ...
                                                            <td class="align-middle">
                                                                <c:choose>
                                                                    <c:when test="${t.status.equals('Rejected')}">
                                                                        <a href="edittopic?id=${t.topicId}"  class="text-secondary font-weight-bold" data-toggle="tooltip" data-original-title="Edit user">
                                                                            View
                                                                        </a>
                                                                    </c:when>
                                                                    ...
                                                                    <c:otherwise>
                                                                        <a href="viewtopicdetail?id=${t.topicId}" class="text-secondary font-weight-bold" data-toggle="tooltip" data-original-title="Edit user">
                                                                            View
                                                                        </a>
                                                                    </c:otherwise>
                                                                </c:choose>
                                                ...
                                                <div aria-label="Page navigation ">
                                                    <ul class="pagination justify-content-center">
                                                        <c:if test="${requestScope.index>1}">      <!--previous button-->
                                                            <li class="page-item"><a class="page-link" href="topiclist?index=${requestScope.index-1}">&laquo;</a></li>
                                                            </c:if>
                                                            <c:forEach begin="1" end="${requestScope.page}" var="i">
                                                            <li class="page-item <c:if test="${i.equals(requestScope.index)}">active</c:if>">
                                                                <a class="page-link" href="topiclist?index=${i}">${i}</a>
                                                            </li>
                                                        </c:forEach>
                                                        <c:if test="${requestScope.index<requestScope.page}">
                                                            <li class="page-item"><a class="page-link" href="topiclist?index=${requestScope.index+1}">&raquo;</a></li>
                                                            </c:if>
                                                    </ul>
                                                </div>
                                            </c:if> 
                                            <c:if test="${requestScope.check.equals('filter')}">  
                                                <div aria-label="Page navigation ">
                                                    <ul class="pagination justify-content-center">
                                                        <c:if test="${requestScope.index>1}">      <!--previous button-->
                                                            <li class="page-item"><a class="page-link" href="topicfilter?level=${requestScope.level}&type=${requestScope.type}&index=${requestScope.index-1}">&laquo;</a></li>
                                                            </c:if>
                                                            <c:forEach begin="1" end="${requestScope.page}" var="i">
                                                            <li class="page-item <c:if test="${i.equals(requestScope.index)}">active</c:if>">
                                                                <a class="page-link" href="topicfilter?level=${requestScope.level}&type=${requestScope.type}&index=${i}">${i}</a>
                                                            </li>
                                                        </c:forEach>
                                                        <c:if test="${requestScope.index<requestScope.page}">
                                                            <li class="page-item"><a class="page-link" href="topicfilter?level=${requestScope.level}&type=${requestScope.type}&index=${requestScope.index+1}">&raquo;</a></li>
                                                            </c:if>
                                                    </ul>
                                                </div>
                                            </c:if>
                                            <c:if test="${requestScope.check.equals('search')}">  
                                                <div aria-label="Page navigation ">
                                                    <ul class="pagination justify-content-center">
                                                        <c:if test="${requestScope.index>1}">      <!--previous button-->
                                                            <li class="page-item"><a class="page-link" href="topicsearch?search=${requestScope.search}&index=${requestScope.index-1}">&laquo;</a></li>
                                                            </c:if>
                                                            <c:forEach begin="1" end="${requestScope.page}" var="i">
                                                            <li class="page-item <c:if test="${i.equals(requestScope.index)}">active</c:if>">
                                                                <a class="page-link" href="topicsearch?search=${requestScope.search}&index=${i}">${i}</a>
                                                            </li>
                                                        </c:forEach>
                                                        <c:if test="${requestScope.index<requestScope.page}">
                                                            <li class="page-item"><a class="page-link" href="topicsearch?search=${requestScope.search}&index=${requestScope.index+1}">&raquo;</a></li>
                                                            </c:if>
                                                    </ul>
                                                </div>
                                            </c:if>  
