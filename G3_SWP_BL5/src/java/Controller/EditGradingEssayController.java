 ...
 * @author AnhTH
 */
@WebServlet(name = "EditGradingEssayController", urlPatterns = {"/editGradingEssay"})
public class EditGradingEssayController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try ( PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet EditGradingEssayController</title>");
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet EditGradingEssayController at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if (userSession == null || userSession.getRole().getRole_id() != 3) {
            ...
            int evaluatorEssayID = Integer.parseInt(request.getParameter("evaluatorEssayID"));

            EvaluatorEssayDAO evaluatorEssayDAO = new EvaluatorEssayDAO();
            EvaluatorEssay evaluatorEssay = evaluatorEssayDAO.getEvaluatorEssayByID(evaluatorEssayID);
            if (evaluatorEssay == null) {

                request.setAttribute("messError", "Error: Essay not exist.");
                request.getRequestDispatcher("editGradingEssay.jsp").forward(request, response);
                return;
            }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (userSession == null || userSession.getRole().getRole_id() != 3) {
            ...
            EvaluatorEssayDAO evaluatorEssayDAO = new EvaluatorEssayDAO();
            boolean updated = evaluatorEssayDAO.updateEvaluatorEssay(evaluatorEssay);
